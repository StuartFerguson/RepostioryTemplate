...
var depositMadeEventHandler = function (s, e) {
    ...
    // emit an balance changed event here
    s = emitBalanceChangedEvent(e.data.aggregateId, eventId, s, e.data.amount, e.data.depositDateTime, "Merchant Deposit");
};
...
var transactionHasCompletedEventHandler = function (s, e) {
    ...
    if (e.data.isAuthorised) {
        ...
        if (amount > 0) {
            var eventId = createEventId(JSON.stringify(e.data));
            s = emitBalanceChangedEvent(e.data.aggregateId, eventId, s, amount * -1, completedTime, "Transaction Completed");
        }
    }
    else {
...
var merchantFeeAddedToTransactionEventHandler = function (s, e) {
    ...
    var eventId = createEventId(JSON.stringify(e.data));
    // emit an balance changed event here
    s = emitBalanceChangedEvent(e.data.aggregateId, eventId, s, e.data.calculatedValue, e.data.feeCalculatedDateTime, "Transaction Fee Processed");
}
